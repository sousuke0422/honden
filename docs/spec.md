...
2026-08-26 時点。shogun（現行）と honden（流れを汲んで別に起こしたもの）が並走している段の整理。
...
```
honden decision raise <<'EOF'        上役だけ
  question: PR#32 を入れ直すか
  choices: ["いま入れる", "次の区切りまで待つ", "取り下げる"]
  fallback: 次の区切りまで待つ
  until: 08:00
EOF
honden decisions                     開いておるものだけ
honden decide 1 "取り下げる" --note "…"    一語で下ろす（将軍だけ）
```
...
| 決め | なぜ |
|---|---|
| 選択肢を 2 つ以上並べる | 自由文は決裁の問いではない。**一語で再開できる形**にする |
| 決まらぬ時どうなるかを問う側が書く | 現行の 🚨 は「無視したら何が起きるか」を言わぬゆえ、積む側に痛みが無い |
| 既定を置くなら期限も要る | 期限の無い既定は、待ち続けるか、いつの間にか倒れるかのどちらかになる |
| 倒れと裁定を分ける | 同じ扱いにすると「殿の判断か時間切れか」が後から分からぬ |
| 二度は下ろせぬ | 覆すなら新しく上げる。跡を書き換えると、いつ何が決まったか辿れぬ |
| 下ろせば一覧から消える | ここが眼目。**絞り込みで消える**——判断が要らぬ |

`dashboard` を手で書き換えるのをやめ、正本から出すための一歩でもある。
