...
# **巡りごとに違う名を与える。** 固定の名にすると、己の別の巡りと陣を
# 取り合う——二つが同時に走れば後の者が「同じ名の陣がある」で倒れ、
# `setup_file failed` として現れる（同時に二つ走らせて実測 2026-08-30）。
# 畳む手は持たぬ（D006）ゆえ、衝突は「避ける」しかない。
#
# `$$` では駄目である——bats は**試験ごとに書を読み直す**ゆえ、立てた時と
# 測る時で別の名になり、陣が見つからぬ（直して即座に赤くなった）。
# 巡りの間ずっと同じで、巡りが違えば違う値が要る。
SOCK="hondenpanes-${BATS_RUN_TMPDIR##*/}"
