...
Bun と Rust は建てるときだけ要る。
出し物から降ろすなら `curl` で足りる。
...
上書きではなく置き換える。
陣が立っている間、芯は自分の binary を掴んでいるので `cp` は `Text file busy` で倒れる。
`mv` なら名札を差し替えるだけなので、走っている側は古い実体を持ったまま生き続け、次に立つときから新しくなる。
...
`first_setup.sh` がここまでやる。
手で入れ替えるときは次のとおり。

**名簿が空のままでは何も送れない。**
誰が何の CLI でどのモデルを使うかを `config/settings.yaml` に書き、正本へ写す。
雛形は `config/settings.yaml.example` にある。
...
| | 命令 | |
|---|---|---|
| 司令を出す | `honden cmd new` | 受け入れ条件つき。全条件が証拠で覆われるまで閉じられない |
| 任を振る | `honden task assign` | 持ち場が重なれば振れない（`claim`） |
| 報せる | `honden report submit` → `report qc` | 足軽から軍師、家老へと自ずと流れる |
| 裁定を仰ぐ | `honden decision raise` / `decide` | 殿の裁可待ちは戦況の頭に出る |
| 戦況を見る | `honden dashboard [--serve]` | 生成物は作らない。読む時に組む |
| 様子を見る | `honden status` | 「不在」と「待機」を言い分ける |
| 起こす | `honden nudge` | 芯が正本の動きを拾って自ずと叩く |
| 報せを撃つ | `honden notify` | 卓上の通知と ntfy。二度は撃たない |
...
`honden help` で全部、`honden help <名>` で一つ出る。
...
エージェント同士は直接つながらない。
やり取りは正本を経由し、変化を芯が拾って tmux 越しに起こす。
**通信路は正本ひとつだけになる。**
...
起こす合図は短い。
`inbox3`（未読 3 件）だけを送り、本文は tmux を通らない。
エージェントが自分で正本を読む。

これは決めであり、門でも支えている。
他人のペインへ手を入れる tmux の副命令（`send-keys`、`respawn-pane`、`paste-buffer`、`run-shell` など）は D014 で塞いである。
読む側（`capture-pane` と `list-panes`）は塞がない。
...
1. **紋様**：D001 から D016。再帰的な削除、`git push --force`、`sudo`、`.gitignore` を迂回する `git add -f`、秘密鍵の読み出しなど。**文字列をそのまま見るだけ**で、取りこぼしはあっても取り過ぎはしない
2. **構文**：Rust の解析器（brush-parser）で命を単位へ割り、**後で実行される命を取り出す**。`if`、`for`、`$()`、改行、`{ }` で紋様を跨がせない

二段目が本務である。
問いは「これは危ない形か」ではなく、**「この語は、後で命として実行されるか」**である。

```
sh -c 'rm -rf /'              引用の中は文字ではなく命
find /tmp -exec rm -rf / \;   -exec の後ろは命
env -i rm -rf /               包み。中の命が本体
chroot / rm -rf /             同上
```

包みは名の一覧ではなく**旗の仕様**で読む。
文字列で渡された命は解き直して再帰する。
**解けなければ通さない** —— 命の名が `$SHELL` のように展開に依るとき、深すぎるとき、多すぎるときは拒む（fail-closed）。

> 誤検知は「実物 14,131 通りで零」と測ったことがあるが、**それは誤検知の軸だけを測ったもので、回避の軸は測っていなかった**。
> 実際、その後の監査で `env rm -rf /` と `sh -c 'rm -rf /'` が素通りしていた。
> いまは通らぬはずの形を意図して作って撃つ釘を持っている。

### 守るもの、守らないもの

**守るのは事故であって、悪意ではない。**

三度の監査で、次がすべて素通りすることを実測した。

```
python3 -c "import os; os.system('…')"   別の言語の中は読めない
node -e "…"  /  perl -e "…"  /  awk 'BEGIN{system("…")}'
vim -c '!…' -c q                         編集器の中から起こす
echo '…' | sh                            文字列が管を通って shell へ入る
base64 で包んだもの                      走らせるまで中身が定まらない
```

最後の二つは**列挙では原理的に届かない**。
ほかも、言語処理系をひとつ封じれば次が残る。

だから**これは堀であって城壁ではない**。
本当に隔てたければ LXC か systemd container を使う（#12 で追う）。

打ち間違いや反射で打つ危険な命は止まる。
隠そうとする者は止まらない。
**その線を書いておくことが、この層にできる一番のことである。**
...
honden guard denials                            # 同じ紋様が叩かれ続けていないか
```
...
| 向き | |
|---|---|
| 陣から携帯 | 裁可待ち、司令の完了、任の失敗、連続日数を撃つ |
| 携帯から陣 | 送った文が将軍の inbox へ「申し出」として入る |
...
理由は「公開しているから」ではない。
**設定と履歴が、そもそも秘密の置き場ではないからである。**
private でも clone され、log に残り、後から公にもなる。
可視の状態に依った守りは、可視が変わった日に崩れる。

> topic は合鍵そのものである。
> 知る者は誰でも読め、誰でも書ける。
> 短い名やありふれた名は honden が起動時に戒める。
> `config/settings.yaml` は `.gitignore` で除外してある。

受け口は来た文を司令として扱わない。
topic 一枚で届いた文には錨も素性の確かめも門も通っていないので、将軍の inbox へ入れるだけにする。
動くのは将軍で、その手はすべて門を通る。
宛先も将軍に固定してある。
文で宛先を名乗れるようにすれば、合鍵一枚で足軽へ直に命じられることになるからである。
...
`SHA256SUMS` は cosign の keyless（Sigstore）で署名してある。
**鍵は存在しない。**
出すときに GitHub Actions の OIDC で身元を示し、Fulcio が短命の証書を出し、Rekor（公の台帳）に跡が残る。
秘密鍵がどこにも無いので、盗まれる物も回す物も無い。

署名するのは `SHA256SUMS` 一枚だけである。
binary はその紙で縛られているので、紙を縛れば全部が縛られる。

```
署名 → SHA256SUMS → 各 binary
```

`honden update` は既定でこれを検め、通らなければ一つも置かない。
`cosign` が無ければ降ろすことを断る。
「あれば検める」にすると、無い機体では黙って素通りになるからである。
どうしても急ぐときの抜け道は `--insecure-skip-signature`（長く醜い名にしてある）。
...
> **身元の指定を省いてはならない。**
> 省くと、Sigstore で署名された物なら誰の物でも通る。
> 誰でも自分の workflow で署名できるので、それは検めになっていない。
> 「署名がある」は「我らが署名した」ではない。
> 札（`refs/tags/`）に縛るのも同じ理由である。
> 枝から走らせた物を認めれば、枝へ書ける者が通る署名を作れてしまう。

#### cosign は v3 以上が要る

束の形が版で変わる。
出す側は v3 で署名し、v3 の束を書く。
v2 の cosign はそれを素では読めない。

ここが罠になる。
**同じ `apt install cosign` でも、入る物が違う。**
...
外からは見分けられない。
だから honden は順で当てようとせず、入れてから検める。
足りなければ次の手（brew か dnf）へ回る。

honden は版まで見て、古ければ告げて止まる（版が読めない時も止まる）。
黙って素通りにはしない。
`HONDEN_COSIGN` で別の場所を指せる。

仕度が cosign を入れるときは、Linux でも brew を先に試す。
brew は上流に近いからである。
ただしこれは当たる率を上げるだけの前置きで、決め手は入れた後の検めである。
土台の道具（tmux や git の類）は逆に土地の手を先に試す。
brew は `/home/linuxbrew` に入り、土地の物を覆い隠すからである。
...
初回の仕度でも同じく効く。
cosign が無ければ、仕度はまず入れてよいかを訊く（`brew`、`apt`、`dnf`。tmux や curl と同じ扱い）。
断られるか、入れられなければ `--fetch` は断る。
その断り文はまず `--build` を勧める。
そちらは cosign を一切要さず、しかもより確かである（clone した source そのものから建てるので、出し物を信じる必要すらない）。

##### 信の根はどこにあるか

`--build` の信の根は `git clone`、すなわち GitHub と TLS である。
`--fetch` も入口は同じで、身元を縛る正規表現は clone した script の中にある。

では署名は何を足しているのか。
**後から差し替えられないこと**と、公の台帳（Rekor）に跡が残ることである。
TLS は「いま通信している相手」しか守らない。
署名は「この紙は、あの日あの札から走った我らの workflow が出した」を、後から誰でも確かめられる形で残す。

だから cosign を入れるのは、降ろした物を信じる前に一度だけ払う手間であって、毎回の負担ではない。
それが厭なら `--build` を選べばよい。
逃げ道ではなく、むしろ堅いほうの道である。

### 出し方

札（`v0.2.0` の形）を打つと GitHub Actions が二つの土地（linux-x64 と linux-arm64）で建てて出す。

**macOS 向けは配っていない。**
芯（`honden-watch`）は inotify ひとつで書いてあり、Linux にしか実装がない（kqueue は別の普請）。
`honden update` も `--fetch` も、macOS では「配っていない」と正直に断る。
404 を掴ませるよりそのほうが親切である。

出す前に札と `src/version.ts` の版を照らし、食い違えば出さない。
揃え忘れは必ず起きるので、人ではなく機械に拒ませる。

### 仮の版（rc、beta、alpha）

版の比べは SemVer 2.0.0 の順序に従う。
ゆえに次がすべて正しく出る。
...
- **仮の版は、同じ数の正式版より低い**：`1.0.0-rc.1` を使っていれば、`1.0.0` が出たときに「新しいものがある」と分かる
- **数の札は数として比べる**：`beta.11` は `beta.2` より新しい（字で比べると逆になる）
- 読めない札（`latest`、`v1.2`、頭に 0 の付いた `01.2.3`）は「新しくない」と見る。壊れた札ひとつで全員が降りてこないようにするためである

そして `-` を含む札は prerelease として出す。
GitHub の `/releases/latest` は prerelease を外して返すので、`honden update` に alpha が降ってくることはない。
試すときは手で降ろす。
...
渡す。
含めないと将軍だけ合図の射程外になり、直訴も夜間の escalation も静かに届かなくなる。
...
| | |
|---|---|
| `NOTICE.md` が**無い** | この repo の MIT。`sousuke0422` が権利者 |
| `NOTICE.md` が**在る** | 他所から来たもの。**免許はその紙に書いてある** |

**「紙が在る＝MIT ではない」ではない。**
はじめそう書いたが、誤りであった（点検が釣った・2026-09-01）。
`skills/vendor/find-skills` は MIT だが、権利者は Vercel であって
こちらではない。**免許の名が同じでも、権利者が違えば別の免許である。**
...
Apache-2.0 は再許諾を許しておらず、他者の寄与を MIT へ付け替える権をこちらは持たないためである。
各 `NOTICE.md` に出所、版、免許、**こちらが変えた箇所**が書いてあり、Apache-2.0 のものは免許の全文（`LICENSE`）も隣に置いてある。
