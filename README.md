...
札（`v0.2.0` の形）を打つと GitHub Actions が二つの土地
（linux-x64 / linux-arm64）で建てて出す。

**macOS 向けは配っていない。** 芯（`honden-watch`）は inotify ひとつで
書いてあり、Linux にしか実装がない（kqueue は別の普請）。`honden update` も
`--fetch` も、macOS では「配っていない」と正直に断る —— 404 を掴ませるより
そのほうが親切である。
