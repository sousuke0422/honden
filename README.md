...
1. **紋様**：D001 から D016。再帰的な削除、`git push --force`、`sudo`、`.gitignore` を迂回する `git add -f`、秘密鍵の読み出しなど
2. **構文**：Rust の解析器（brush-parser）で命を単位へ割る。`if`、`for`、`$()`、改行、`{ }` で紋様を跨がせない。解けなかった命は通さない（fail-closed）

実物 14,131 通りで測って誤検知は零だった。
それでも堀であって城壁ではない。
