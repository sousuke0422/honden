...
`cmd_abandoned` / `lease_stalled` / `report_unreviewed` / `clear_command` / `guard_appeal` / `guard_grant`
...
2. **Wake-up signal**: 二つの経路がある。
   ...
   - **優先度1 — 横乗せ (ride-along)**: どの副命令の出力にも、呼び出し主に急ぎの未読が
     ...
     急ぎ（`clear_command` / `cmd_new` / `cmd_update` / `cmd_abandoned` / `lease_stalled` / `report_unreviewed` / `guard_appeal` / `guard_grant`）
