...
**変えたのは一行だけ**である——前書きへ `user-invocable: true` を足した。
本文（規範そのもの）には一字も手を入れていない。

手で発火させるための欄である。この書は「日本語で書くとき」に気づかれて回る
性質のものだが、**殿は手でも呼ばれる**——推敲を頼む折に名指しできねば困る。

**既定に頼らず明に書く。** いまの版では書かずとも出るのかもしれぬが、
**出なかった時期がある**（殿の実測 2026-08-31）。既定は版で変わりうるので、
書いておけばどちらに転んでも効く。

Unlicense は変更の記載を求めないが、**次に上流から取り直す者が
「なぜ差があるのか」を辿れるように**書き残す。
