 ...
 * **守るのは事故である。悪意ではない。**
 *
 * 三度の監査で、次がすべて素通りすることを実測した（2026-09-01）。
 *
 * ```text
 * python3 -c "import os; os.system('…')"   別の言語の中は読めぬ
 * node -e "…"  /  perl -e "…"  /  awk 'BEGIN{system("…")}'
 * vim -c '!…' -c q                         編輯器の中から起こす
 * echo '…' | sh                            文字列が管を通って shell へ入る
 * python3 -c "…base64.b64decode(…)…"       走らせるまで中身が判らぬ
 * ```
 *
 * 最後の二つは**列挙では原理的に届かぬ**。走らせる前に中身が定まらぬゆえ。
 * ほかも、言語処理系を一つ封じれば次が残る——`python3` を封じても `node` が
 * あり、`node` を封じても `perl` があり、その先に `awk` がある。
 *
 * ## ゆえに、これは堀である
 *
 * `README.md` は初めから「堀であって城壁ではない。本当に隔てたければ LXC か
 * systemd container を使う」と書いてあった。**その一文が正確だった。**
 * こちらはそれを書きながら、堀を城壁にしようと三度試みた。
 *
 *   一度目  **名**の一覧で包みを剥がす      → `sh -c` は名では捉えられぬ
 *   二度目  **包み**の一覧で命を取り出す    → 一覧の外に十三通り
 *   三度目  一覧を厚くする                  → 一覧の中にも四つ
 *
 * 二度目も一度目と同じ形の誤りであった。**列挙の対象を変えただけで、
 * 列挙に依っていることは変わっていない。**
 *
 * ## それでも置く理由
 *
 * 事故は実際に起きる。`rm -rf /` を打ち間違える、`git push --force` を
 * 反射で打つ、滞った watcher を `pkill` で薙ぐ。**そこは止まる。**
 *
 * 隠そうとする者は止まらぬ。それは隔離の仕事であり、#12 で追う。
 * **守れておらぬ範囲を書いておくことが、いまできる一番のことである。**
 */
...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    // **絶対域とする**（殿の裁可 2026-09-01）。
    //
    // これまでは手形で通していた。`CLAUDE.md` は Tier 1 の絶対禁と定め、
    // README も「エージェントはセッションを畳めぬ」と公に述べておるのに、
    // 実装だけが直訴を許しておった（監査が釣った・2026-09-01）。
    //
    // 手形を残しておった理由は「滞った process を止める道が要る」であった。
    // その道は `honden-kill` として別に開いた——己の系譜の下だけを、
    // pidfd で掴んでから撃つ。**逃げ道が要るから絶対にできぬ、ではなくなった。**
    absolute: true,
  },
  {
    id: 'D007',
