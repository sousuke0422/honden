 ...
 * # 秘密は設定に書かぬ
 *
 * **設定と履歴は、秘密の置き場ではない。** repo が公か private かに依らぬ。
 *
 * はじめ「honden は公開しておるゆえ」と書いていた。理由が弱い
 * （点検が釣った・2026-09-01）——private でも clone され、log に残り、
 * 後から公にもなる。**可視の設定に依った守りは、可視が変わった日に崩れる。**
 *
 * ntfy の topic と合鍵は資格情報である。ゆえに追跡下へ置かぬ。
 * **環境変数から取る**——`NTFY_TOKEN`、または `NTFY_USER` と `NTFY_PASS`。
 * 旧も `config/ntfy_auth.env` を別に持っており、分ける流儀は同じである。
 *
 * # 設定が無ければ名乗り出ぬ
 *
